  ...
  useEffect(() => {
    ...
    class App {
      ...
      onMouseMove(ev) {
        ...
        // Define the center area (20% tolerance of container width/height)
        const toleranceX = rect.width * 0.2;
        const toleranceY = rect.height * 0.2;

        if (Math.abs(x - centerX) < toleranceX / 2 && Math.abs(y - centerY) < toleranceY / 2) {
          // Mouse is near center: trigger speedUp
          if (this.options.onSpeedUp) this.options.onSpeedUp(ev);
          this.fovTarget = this.options.fovSpeedUp;
          this.speedUpTarget = this.options.speedUp;
        } else {
          // Outside center area: revert
